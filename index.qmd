...
💡 ["occurrence of disease in excess of normal"]{.purple.emph}
...
::: {.column width="50%"}
-   Norovirus causes [vomitting]{.purple} and [diarrhoea]{.purple} (you might call this gastro).
...
- Now to our computers
- You might be wondering what R looks like
- The top left is our R code file where we'll be writing our code
- The top right is our environment pane which shows us R objects 
- The bottom left pane shows us outputs of our code
- And bottom right currently shows us the folder that we're working

Now get everyone to look at their computer and open screen to code with them 
...
- meaningful or descriptive word
- no spaces
- computer friendly to use underscore if you want to write multiple words 
- try not to spend toooo long on this: max 2 mins 
...
We've set up a pre-written files to load this in for us! 
...
:::{.notes}
Shows them what's on their screen and that we've done this for them already 
Show them how to run these commands by highlighting the ENTIRE code -- easier 
Again try not to spend too long on this
:::
...
<!-- scatterplot(mpg, x = displ, y = cty, fill_col = "violet", title = "My scatterplot") -->
...
<!-- boxplot(mpg, x = class, y = cty, fill_col = "gold", title = "My boxplot") -->
...
scatterplot(outbreak_data, 
            x = class, y = temperature, 
            fill_col = "violet", 
...
## Re-organising our symptoms data
...
```{r bar_sick_2, fig.width=6, fig.height=4}
#| echo: true
#| output-location: fragment
barplot(symptoms_sick, 
        x = symptom, 
        colour_by = status, 
        horizontal = TRUE, 
        title = "What symptoms do the sick students have ?")
```
...
ggmap(melb_map)+
   geom_point(data=map_data, 
              aes(x =long, y = lat, colour = sickness), 
              size = 12, shape = 1)
```
